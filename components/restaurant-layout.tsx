          ...
          <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
            <div className="flex items-center justify-between w-full sm:w-auto">
              <div className="flex items-center space-x-4">
                ...
                <span className="text-xl font-bold hidden sm:inline">Restaurant Panel</span>
              </div>
...
}: { user: any; onLogout: () => void; selectedRestaurant: any; setSelectedRestaurant: (restaurant: any) => void }) {
  return (
    <DropdownMenu>
      ...
      <DropdownMenuContent className="w-64" align="end" forceMount>
        ...
        {user?.restaurants.map((restaurant: any) => (
          <DropdownMenuItem key={restaurant.id} onSelect={() => setSelectedRestaurant(restaurant)}>
        ...
        <div className="px-2 py-1.5 space-y-1">
          <p className="text-xs font-medium text-muted-foreground">
            Роль: {user?.role === "customer" ? "Администратор ресторана" : user?.role}
          </p>
